...
@everywhere function TDM(mat::Array{Float64,2}, t::Array{Float64,1}, set::set_TDM)
    ...
    cut = findfirst(fun_std(t[skip:end],fit_std.param)./ave_t[skip:end] .> set.cutcrit)+skip-1
    set.tcut = t[cut]
...
function bootstrapping(mat, t, set)
    ...
    randmat = Array{Array{Int64,1},1}(undef,Int64(1.5*nboot))
    for i = 1:Int64(1.5*nboot) randmat[i] = sort(rand(1:nsim,10)) end
    ...
    plot!(x,y,label="Fit")
    outfolder = string(set.folder,"/TransportProperties/")
    png(string(outfolder,"histogram.png"))
